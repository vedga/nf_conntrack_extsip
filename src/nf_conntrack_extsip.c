...
#define SIP_MAX_CALLID_LIFETIME (24 * 60 * 60)
...
MODULE_DESCRIPTION("Extended SIP connection tracking helper");
MODULE_ALIAS("ip_conntrack_extsip");
MODULE_ALIAS_NFCT_HELPER("extsip");
...
static int extsip_help_tcp(struct sk_buff *skb, unsigned int protoff,
...
static int extsip_help_udp(struct sk_buff *skb, unsigned int protoff,
			struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	unsigned int dataoff, datalen;
	const char *dptr;
...
static int __init nf_conntrack_sip_init(void)
{
	...
	for (i = 0; i < ports_c; i++) {
		...
		sip[i][0].help = extsip_help_udp;
		sip[i][1].tuple.src.l3num = AF_INET;
		sip[i][1].tuple.dst.protonum = IPPROTO_TCP;
		sip[i][1].help = extsip_help_tcp;

		sip[i][2].tuple.src.l3num = AF_INET6;
		sip[i][2].tuple.dst.protonum = IPPROTO_UDP;
		sip[i][2].help = extsip_help_udp;
		sip[i][3].tuple.src.l3num = AF_INET6;
		sip[i][3].tuple.dst.protonum = IPPROTO_TCP;
		sip[i][3].help = extsip_help_tcp;
...
static int __init nf_conntrack_extsip_init(void);
static void nf_conntrack_extsip_shutdown(void);
static void on_nf_conn_destroy(struct nf_conn *ct);
...
unsigned int sip_timeout __read_mostly = SIP_TIMEOUT;
module_param(sip_timeout, uint, 0600);
MODULE_PARM_DESC(sip_timeout, "timeout for the master SIP session");

unsigned int sip_max_callid_lifetime __read_mostly = SIP_MAX_CALLID_LIFETIME;
module_param(sip_max_callid_lifetime, uint, 0600);
MODULE_PARM_DESC(sip_max_callid_lifetime, "Max callid lifetime");
...
/* Хранилище элементов типа extsip_callinfo */
struct kmem_cache *extsip_callinfo_cachep __read_mostly;

static struct nf_conntrack_helper extsip[MAX_PORTS][4] __read_mostly;

static const struct nf_conntrack_expect_policy extsip_exp_policy[EXTSIP_EXPECT_MAX + 1] = {
...
static int __init nf_conntrack_extsip_init(void) {
    ...
    /* Регистрируем net namespace init/exit entryes */
    ret = register_pernet_subsys(&extsip_net_ops);

    if(ret < 0) {
        /* Не удалось зарегистрировать net namespace init/exit entryes */
        return ret;
    }

    extsip_callinfo_cachep = kmem_cache_create("nf_conntrack_extsip",
                            sizeof(struct extsip_callinfo),
                            0, 0, NULL);
    
    if(!extsip_callinfo_cachep) {
        /* Освобождаем выделенные ранее ресурсы */
        unregister_pernet_subsys(&extsip_net_ops);
...
//    pr_info("nf_conntrack_extsip_init - set def port\n");
    
    for (i = 0; i < ports_c; i++) {
//        pr_info("nf_conntrack_extsip_init port #%u\n", i);
    
        memset(&extsip[i], 0, sizeof(extsip[i]));

        extsip[i][0].tuple.src.l3num = AF_INET;
        extsip[i][0].tuple.dst.protonum = IPPROTO_UDP;
        extsip[i][0].help = extsip_help_udp;
        extsip[i][0].destroy = on_nf_conn_destroy;
        extsip[i][1].tuple.src.l3num = AF_INET;
        extsip[i][1].tuple.dst.protonum = IPPROTO_TCP;
        extsip[i][1].help = extsip_help_tcp;
        extsip[i][1].destroy = on_nf_conn_destroy;

        extsip[i][2].tuple.src.l3num = AF_INET6;
        extsip[i][2].tuple.dst.protonum = IPPROTO_UDP;
        extsip[i][2].help = extsip_help_udp;
        extsip[i][2].destroy = on_nf_conn_destroy;
        extsip[i][3].tuple.src.l3num = AF_INET6;
        extsip[i][3].tuple.dst.protonum = IPPROTO_TCP;
        extsip[i][3].help = extsip_help_tcp;
        extsip[i][3].destroy = on_nf_conn_destroy;

        for (j = 0; j < ARRAY_SIZE(extsip[i]); j++) {
            extsip[i][j].data_len = sizeof(struct nf_ct_extsip_master);
            extsip[i][j].tuple.src.u.udp.port = htons(ports[i]);
            extsip[i][j].expect_policy = extsip_exp_policy;
            extsip[i][j].expect_class_max = EXTSIP_EXPECT_MAX;
            extsip[i][j].me = THIS_MODULE;

            if (ports[i] == SIP_PORT)
                    sprintf(extsip[i][j].name, "extsip");
            else
                    sprintf(extsip[i][j].name, "extsip-%u", i);

            pr_info("EXTSIP: Register policy %s for %s %s port %u\n", 
                    extsip[i][j].name,
                    (extsip[i][j].tuple.src.l3num == AF_INET ? "IPv4" : "IPv6"),
                    (extsip[i][j].tuple.dst.protonum == IPPROTO_UDP ? "UDP" : "TCP"),
                    ports[i]);

            ret = nf_conntrack_helper_register(&extsip[i][j]);
            if (ret) {
                    pr_err("nf_ct_sip: failed to register"
                           " helper for pf: %u port: %u\n",
                           extsip[i][j].tuple.src.l3num, ports[i]);
                    nf_conntrack_extsip_shutdown();
                    return ret;
            }
        }
    }
...
static void nf_conntrack_extsip_shutdown(void) {
    ...
    for (i = 0; i < ports_c; i++) {
        for (j = 0; j < ARRAY_SIZE(extsip[i]); j++) {
            if (extsip[i][j].me == NULL)
                    continue;
            nf_conntrack_helper_unregister(&extsip[i][j]);
            
            pr_info("EXTSIP: Unregister policy %s for %s %s port %u\n", 
                    extsip[i][j].name,
                    (extsip[i][j].tuple.src.l3num == AF_INET ? "IPv4" : "IPv6"),
                    (extsip[i][j].tuple.dst.protonum == IPPROTO_UDP ? "UDP" : "TCP"),
                    ports[i]);
...
//    rcu_barrier(); /* Wait for call_rcu() before destroy */
    kmem_cache_destroy(extsip_callinfo_cachep);
...
/**
 * Called on ct being removed
 * 
 * @param ct
 * 
 * Вызывается непосредственно перед удалением ct из всех списков
 */
static void on_nf_conn_destroy(struct nf_conn *ct) {
    ...
    hash_for_each_safe(master->callinfo, bkt, tmp, ci, ci_link) {
...
void trace_pkt(const char* label, struct sk_buff *skb, struct extsip_callinfo *callinfo) {
    struct iphdr *ip_header = (struct iphdr *)skb_network_header(skb);
    unsigned int src_ip = (unsigned int)ip_header->saddr;
    unsigned int dest_ip = (unsigned int)ip_header->daddr;
    
    pr_info("EXTSIP: %s %pI4 -> %pI4 Call-ID: %s\n", label, &src_ip, &dest_ip, callinfo->callid);
}

module_init(nf_conntrack_extsip_init);
module_exit(nf_conntrack_extsip_shutdown);
#endif
